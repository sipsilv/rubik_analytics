...
export function ViewConnectionModal({ isOpen, onClose, connection }: ViewConnectionModalProps) {
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
            <div 
                className="bg-[#121b2f] border border-[#1f2a44] rounded-lg shadow-xl w-full max-w-2xl mx-4 p-6 max-h-[90vh] overflow-y-auto relative"
                style={{
            ...
            >
                ...
                <button 
                    onClick={onClose} 
                    className="absolute top-0 -right-12 z-[100] w-8 h-8 p-0 bg-transparent hover:bg-red-600 rounded text-red-600 hover:text-white transition-colors flex items-center justify-center"
                    title="Close"
                    aria-label="Close"
                    style={{
                        animation: isVisible ? 'modalFadeIn 0.3s cubic-bezier(0.4, 0, 0.2, 1)' : 'none',
